...
node 10 [expr $d/2.]	[expr -$ln/2.]
node 20 [expr $d/2.]	[expr 3.*$ln/2.]
...
set fy [expr 63.*$ksi]
#set fu [expr 1.2*$fy]; # From webPlotDigitizer
set fu [expr 90.*$ksi]; # From webPlotDigitizer
set epsy [expr $fy/(29000.*$ksi)]
set eu 0.12; # From webPlotDigitizer

set lambda [expr 3.*63000./(40.*sqrt(5990.)*2.5)]
set db [expr 0.875*$in]

set ld [expr $lambda*$db]

set lambda_e [expr 528./63.]; #Koorosh
...
puts "eu $eu"
puts "eps_bar/epsy [expr $eps_bar/$epsy]"

set Keff [expr $D/($D+$db*$lambda)]
set fact [expr 2.*$db*$lambda2/$D]
...
#SteelDRC parameters 
set esh 0.01; # From webPlotDigitizer
set eu 0.12; # From webPlotDigitizer
set Psh 3.5
set omegaFac 1.0
...
set elasticMat $matCount
incr matCount

uniaxialMaterial Elastic $elasticMat [expr 1000.*$kip*2.*$ln]

# #####
set bilinMat $matCount
incr matCount

set Ep1 [expr 227.1*$kip*$ln*1.0]
set Ep2 [expr 935.1*$kip*$ln*0.8]
...
# ########### Geometry
element trussSection 1 1 4 $CB_steel
element trussSection 2 2 3 $CB_steel

#geomTransf Linear 1
#element truss 10 10 20 1.0 $elasticMat
element truss 10 10 20 1.0 $bilinMat

#element elasticBeamColumn 10 3 10 [expr $bw*$h] $Ec $I 1
#element elasticBeamColumn 11 4 10 [expr $bw*$h] $Ec $I 1
...
#Gravity analysis 
set patternTag 0
set P_block [expr 150.*$pcf*36.*$in*48*$in*21.6*$in]
#set P_block 0.
set P_beam [expr 150.*$pcf*$bw*$h*$ln/2.]
set P [expr ($P_block + $P_beam)/2.]
#set P [expr ($P_block + $P_beam)/2. + 50.]
#set P [expr ($P_block + $P_beam)/2. + 42.*$ft*$kip/$d]
...
recorder Element -file $dataDir/Rod.txt -ele 10 axialForce
...
recorder Element -file $dataDir/Concrete_1.txt -ele 3 section $CB fiber 0 0 $Conc stressStrain
recorder Element -file $dataDir/Concrete_2.txt -ele 4 section $CB fiber 0 0 $Conc stressStrain
