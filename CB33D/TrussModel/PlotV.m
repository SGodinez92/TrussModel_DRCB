...
testraw = load('Naish.txt');
% testEtabs = load('Naish_etabs.txt');
% testEtabs = load('CB33D_etabs_beta0.7.txt');
testEtabs = load('CB33D_etabs_beta1.0.txt');
L = 60;
x = [-0.08 0.08];
% y = [-150 150];
y = [-600 600];

testraw(:,2) = testraw(:,2)/L;

lastStep = 1861;
test = testraw(1:lastStep,:);
test(:,1) = test(:,1);
...
plt0 = plot(test(:,2),test(:,1)*4.4482,'linewidth',2.5,'color',[0.75 0.75 0.75]);
...
% plt2 = plot(modelFile2(:,2)/L,modelFile2(:,1),'r');
% legend([plt1 plt2],'K_o','K_{eff}','location','southeast')
...
% Energy comparison
% Test
aux = find(test(:,2) > 0.0199,1);
testE = test(1:aux,:);
aux = find(testE(:,2)<0,1,'last');
testE = test(aux:end,:);
...
[pksMin,locsMin] = findpeaks(-testE(:,2));
locsMin(pksMin<0.0199) = [];
pksMin(pksMin<0.0199) = [];

locsMin([4 6 9 11 12 14 16 17 19 20]) = [];
pksMin([4 6 9 11 12 14 16 17 19 20]) = [];
...
% Model OS
aux = find(modelFile(:,2)/L > 0.0199,1);
...
locsMinE(pksMinE<0.0199) = [];
pksMinE(pksMinE<0.0199) = [];
...
% Shear Spring
aux = find(modelFile2(:,2)/L > 0.0199,1);
...
% Moment Spring
aux = find(modelFile3(:,2)/L > 0.0199,1);
...
xlabel('Chord Rotation (radians)')
title('Energy Dissipated CB33D')
