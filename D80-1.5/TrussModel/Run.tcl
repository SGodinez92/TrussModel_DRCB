...
set h  	[expr 18.*$in]
set ln 	[expr 27.*$in]			
set alpha	[expr 22.7*$pi/180.]
set As_diag [expr 2.64*$in2]
...
#node 10 [expr $d/2.]	[expr $ln+48.5/2] 
...
set lambda [expr 3.*83000./(40.*sqrt(7600.)*2.5)]
set db [expr 0.75*$in]

set ld [expr $lambda*$db]

set lambda_e [expr 528./83.]; #Koorosh
set lambda_i $lambda_e
...
# #set e2 [expr 0.5*$eu]
# set e2 [expr $eu*$fact]
# #set e2 [expr 2*$lambda2*$db*($eu-$epsy)/$D-$epsy]
# set e3 $eu
# 
# set s1 $fy
# #set s2 $fy
...
#Gravity analysis 
set patternTag 0
set P_block [expr 150.*$pcf*36.*$in*48.5*$in*22.*$in]
#set P_block 0.
