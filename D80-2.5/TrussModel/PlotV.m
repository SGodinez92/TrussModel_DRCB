...
testE = load('D80-2.5_partial.txt');
testE(:,1) = testE(:,1)/100;
test = load('D80-2.5.txt');
% testEtabs = load('D80-2.5_etabs_beta0.7.txt');
testEtabs = load('D80-2.5_etabs_beta1.0.txt');
L = 45;
x = [-0.08 0.08];
% y = [-250 250];
y = [-1200 1200];
...
% legend([plt0,plt1,plt2,plt3],'Test','Model OS','Model ETABS','Shear Spring','location','southeast')
...
[pksE,locsE] = findpeaks(testPartialE(:,1));
locsE(pksE<0.0199) = [];
pksE(pksE<0.0199) = [];
flag = 0;
flag2 = 0;
...
[pksMinE,locsMinE] = findpeaks(-testPartialE(:,1));
...
title('Energy Dissipated D80-2.5')
